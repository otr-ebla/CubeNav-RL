...
def train_tqc_agent(num_rays, model_path="xml_models/obstacles_cube_model.xml", num_envs=16, num_steps=100000, run_id="training1", training=True):
    log_dir = "./tqc_cube_tensorboard/"
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=500000,
        save_path="./tqc_cube_checkpoints/",
        name_prefix="tqc_cube_model"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train a TQC agent to navigate a cube to a target sphere.")
    ...
    parser.add_argument("--train", action="store_true", help="Train the TQC agent.")
    parser.add_argument("--eval", action="store_true", help="Evaluate the TQC agent.")
    ...
    if train:
        train_tqc_agent(args.num_rays, args.model_path, args.num_envs, args.num_steps, args.run_id, training=True)
    
    # Load the last trained model
    model = TQC.load(f"{args.run_id}")
